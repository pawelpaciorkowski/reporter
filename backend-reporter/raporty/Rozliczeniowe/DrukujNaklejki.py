...
SQL_SIMPLE = """
    ...
    where d.typdok_idn like 'FV_B%' and a.datdok between  ? and ? and left(i.stwystfakt_ids,3) like ?
...
def report_print(task_params):
    ...
    if params['src'] == 'simple':
        ...
        sd = SimpleDatasource()
        rows = sd.dict_select(SQL_SIMPLE, (datetime.datetime.strptime(params['dataod'], "%Y-%m-%d"), datetime.datetime.strptime(params['datado'], "%Y-%m-%d"), stanowisko))
    elif params['src'] in ('snrf', 'snrr'):
        podnazwa = params['lab']
        snr = SNR()
        if params['src'] == 'snrf':
